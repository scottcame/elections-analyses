...
precincts2024 %>% filter(LegDistrict %in% primaryGeneralDistricts) %>% left_join(primaryGeneralAdf, by=c('FIPS', 'Precinct'='PrecinctCode')) %>% ggplot() +
  ...
  geom_sf(data = primaryGeneralLdBoundaries, fill = NA, color = "gray80", linewidth = 0.2) +
  geom_text(data = primaryGeneralLdLabels, aes(x = x, y = y, label = LegDistrict), size = 2.5, color = "black", alpha = 0.5) +
